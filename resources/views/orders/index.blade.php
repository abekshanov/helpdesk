...
@section('content')
    @can('create', App\Order::class)
        <a type="button" href="{{route('orders.create')}}" class="btn btn-outline-success">Создать заявку</a>
    @endcan

    @can('filter', App\Order::class)
    ...
    <table class="table table-hover my-3">
        ...
        @foreach ($orders as $order)
            <tr>
                ...
                <td>
                    <div @if ($order->status == config('helpdesk.status.closed')) class="badge badge-success px-2 py-1"@endif>
                    {{$order->status}}
                    </div>
                </td>
                <td>
